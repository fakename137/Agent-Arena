...
// Remove the problematic style import if it's just globals.css
// import style from '../styles/globals.css'; // Remove this line
// If you have specific styles for this component, create a CSS Module (e.g., BasementBattle.module.css)
// and import it like: import styles from './BasementBattle.module.css';

const BasementBattle = ({
  bossHealth,
  remyHealth,
  combatLog,
  winner, // This prop should hold the name of the winner (e.g., 'Brad' or 'Remy') or null/undefined
  executeAttack,
  executeCombo,
  toggleBlock,
  dodge,
  taunt,
  specialMove,
  // Expecting blocking to be an object like { boss: boolean, remy: boolean }
  blocking = { boss: false, remy: false }, // Default prop value
  loading,
}) => {
  // Ensure blocking has the expected structure even if undefined is passed
  const safeBlocking = {
    boss: blocking?.boss || false,
    remy: blocking?.remy || false,
  };

  // Derive winner message if winner prop is a string name
  const winnerMessage = winner ? `${winner.toUpperCase()} WINS!` : null;
  ...
  return (
    <div style={{ position: 'relative', width: '100vw', height: '100vh' }}>
      ...
      {/* --- Controls Section --- */}
      {/* Ensure buttons are disabled based on loading, health, and attacking state if needed */}
      {/* Boss controls */}
      <div
        style={{
          position: 'absolute',
      ...
      >
        ...
        >
          {/* Use safeBlocking */}
          {safeBlocking.boss ? 'Stop Blocking' : 'Block'}
        </button>
        <button
          onClick={() => dodge('boss')}
          disabled={loading || bossHealth <= 0 || remyHealth <= 0}
      ...
      >
        ...
        >
          {/* Use safeBlocking */}
          {safeBlocking.remy ? 'Stop Blocking' : 'Block'}
        </button>
        <button
          onClick={() => dodge('remy')}
          disabled={loading || bossHealth <= 0 || remyHealth <= 0}
      ...
      {/* Winner announcement */}
      {/* Display winner if winner prop is truthy */}
      {winnerMessage && (
        <div
          style={{
            position: 'absolute',
